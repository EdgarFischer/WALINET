#!/usr/bin/env python3
"""Extract and cross-check stored Vienna csi_path values without modifying them."""
...
RESULTS_ROOT = Path(
    ...
    "LargeData_d3hj/Results_Dat_LCM_SameProcessingAsOnline/Vienna"
)
VOLUME_DIRECTORY_PATTERN = re.compile(r"^Vol0?([5-9])_Dat.*$")
...
def main() -> None:
    ...
    for volume, entries in sorted(variants.items()):
        if len({path for _, path in entries}) > 1:
            ...
            conflicts.append(f"Vol{int(volume):02d} has different paths:\n{details}")
    ...
    for volume, entries in sorted(variants.items(), key=lambda item: int(item[0])):
        print(f"Vol{int(volume):02d}: {entries[0][1]}")

    all_found = all(
        Path(entries[0][1]).is_file()
